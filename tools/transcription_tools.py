...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    if provider == "local":
        ...
        return _apply_stt_language_guard(_transcribe_local(file_path, model_name), stt_config)
    ...
    if provider == "local_command":
        ...
        return _apply_stt_language_guard(_transcribe_local_command(file_path, model_name), stt_config)

    if provider == "groq":
        model_name = model or DEFAULT_GROQ_STT_MODEL
        return _apply_stt_language_guard(_transcribe_groq(file_path, model_name), stt_config)

    if provider == "openai":
        openai_cfg = stt_config.get("openai", {})
        model_name = model or openai_cfg.get("model", DEFAULT_STT_MODEL)
        return _apply_stt_language_guard(_transcribe_openai(file_path, model_name), stt_config)

    if provider == "mistral":
        mistral_cfg = stt_config.get("mistral", {})
        model_name = model or mistral_cfg.get("model", DEFAULT_MISTRAL_STT_MODEL)
        return _apply_stt_language_guard(_transcribe_mistral(file_path, model_name), stt_config)

    if provider == "xai":
        # xAI Grok STT doesn't use a model parameter — pass through for logging
        model_name = model or "grok-stt"
        return _apply_stt_language_guard(_transcribe_xai(file_path, model_name), stt_config)
