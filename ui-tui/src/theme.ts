...
function shouldNormalizeAnsiLightTheme(env: NodeJS.ProcessEnv = process.env): boolean {
  const colorTerm = (env.COLORTERM ?? '').trim().toLowerCase()
  const termProgram = (env.TERM_PROGRAM ?? '').trim()

  return termProgram === 'Apple_Terminal' && colorTerm !== 'truecolor' && colorTerm !== '24bit' && detectLightMode(env)
}
...
export function normalizeThemeForAnsiLightTerminal(
  ...
  if (!shouldNormalizeAnsiLightTheme(env)) {
    return theme
  }
...
export const DEFAULT_THEME: Theme = normalizeThemeForAnsiLightTerminal(detectLightMode() ? LIGHT_THEME : DARK_THEME)
